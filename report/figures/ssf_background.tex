...
\begin{tikzpicture}[
    ...
    % 1. Ranks MPI (Données éparpillées en mémoire)
    \node[mpi] (mpi0) at (-3, 2.5) {Rank 0};
    \node[mpi] (mpi1) at (-1, 2.5) {Rank 1};
    \node[mpi] (mpi2) at (1, 2.5) {Rank 2};
    \node (mpidots) at (2.2, 2.5) {$\dots$};
    \node[mpi] (mpiN) at (3.5, 2.5) {Rank $N$};
    
    \node[anchor=west, text=blue!80!black] at (4.5, 2.5) {\textbf{Memory Computation}};
    ...
    \draw[->, >=stealth, line width=2pt, teal!80!black] (0, -0.4) -- (pfsfile.north) 
        node[midway, right, text=black, scale=0.85] {1 contigious data writing flow};
